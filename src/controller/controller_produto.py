...
class Controller_Produto:
    ...
    def atualizar_produto(self) -> Produto:
        ...
        if not self.verifica_existencia_produto(oracle, codigo_produto):
            ...
            oracle.write(f"update produtoss set nome = '{nome}', valor = '{valor}'  where codigo_produto = {codigo_produto}")
            # Recupera os dados do novo produto criado transformando em um DataFrame
            df_produto = oracle.sqlToDataFrame(f"select codigo_produto, nome, valor from produtoss where codigo_produto = {codigo_produto}")
            # Cria um novo objeto produto
    ...
    def excluir_produto(self):
        ...
        if not self.verifica_existencia_produto(oracle, codigo_produto):            
            ...
            oracle.write(f"delete from produtoss where codigo_produto = {codigo_produto}")            
